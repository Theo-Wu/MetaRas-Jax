import functools
import jax.numpy as jnp
import jax
from jax import grad,jit,vmap,jacfwd, jacrev
import numpy as np
from jax.scipy.stats import norm
from functools import partial
from kernels import *
from p2t import point_to_line_distance_parallel

@partial(jax.jit, static_argnums=(1,))
def normalize(v,axis=-1):
    safe_norm = jnp.where(jnp.linalg.norm(v,axis=axis)!=0,jnp.linalg.norm(v,axis=axis),1e-6)
    return v/safe_norm[...,None]

def check_anti_clockwise(a,b,c):
    cx = c[:,0]
    cy = c[:,1]
    ax = a[:,0]
    ay = a[:,1]
    bx = b[:,0]
    by = b[:,1]
    # ax = mesh[:,0,0]
    # ay = mesh[:,0,1]
    # bx = mesh[:,1,0]
    # by = mesh[:,1,1]
    # cx = mesh[:,2,0]
    # cy = mesh[:,2,1]
    return ((cy-by)*(bx-ax)-(by-ay)*(cx-bx) > 0) #[n,]

def p2f(points,polygon_position):
    vertexCount = polygon_position.shape[1]
    min_dis = 1e6
    sign = 1
    for i in range(vertexCount):
        sign *= (edge(points,polygon_position[:,i%vertexCount],polygon_position[:,(i+1)%vertexCount])>0.) #inside positive outside
        p1 = polygon_position[:,i%vertexCount,0:2]
        p2 = polygon_position[:,(i+1)%vertexCount,0:2]
        dis = point_to_line_distance_parallel(points,p1,p2)
        min_dis = jnp.fmin(min_dis,dis)
    sign = 2*sign-1
    check = check_anti_clockwise(polygon_position[:,0],polygon_position[:,1],polygon_position[:,2])
    return min_dis,sign,check
    
def edge(p,a,b):
    h,w,_ = p.shape
    n,_ = a.shape
    px = p[:,:,0]
    py = p[:,:,1]
    ax = a[:,0]
    ay = a[:,1]
    bx = b[:,0]
    by = b[:,1]
    f = px*jnp.broadcast_to((by-ay)[...,None,None],(n,h,w)) - py*jnp.broadcast_to((bx-ax)[...,None,None],(n,h,w)) + jnp.broadcast_to((ay*bx-ax*by)[...,None,None],(n,h,w)) #[n,h,w]
    return -f

def render(distance,sign,check,kernel,bandwidth=1e-7):
    rasterise = get_kernels(sign,distance,kernel,bandwidth) #[n,h,w]
    rasterise *= jnp.where(check!=0,1.0,1e-6)[:,None,None]
    return rasterise #[b,h,w]

def getArea(c,a,b):
    cx = c[:,0]
    cy = c[:,1]
    ax = a[:,0]
    ay = a[:,1]
    bx = b[:,0]
    by = b[:,1]
    f = cx*(by-ay) - cy*(bx-ax) + (ay*bx-ax*by)
    return -f

def getArea_n(polygon_position):
    n,t,_ = polygon_position.shape
    area = 0
    for i in range(1,t-1):
        area += getArea(polygon_position[:,0],polygon_position[:,i],polygon_position[:,i+1])
    return area

def getnormal(mesh):
    if len(mesh.shape)==3: #[n,3,3]
        BA = mesh[:,1,:3]-mesh[:,0,:3]
        CA = mesh[:,2,:3]-mesh[:,0,:3]
        normal = jnp.cross(BA,CA) #[n,3]
    elif len(mesh.shape)==4: #[b,n,3,3]
        BA = mesh[:,:,1,:3]-mesh[:,:,0,:3]
        CA = mesh[:,:,2,:3]-mesh[:,:,0,:3]
        normal = jnp.cross(BA,CA) #[b,n,3]
    return normalize(normal,-1)

def getUV(points,polygon_position):
    area = getArea(polygon_position[:,0],polygon_position[:,1],polygon_position[:,2])[:,None,None]
    e0 = edge(points,polygon_position[:,1],polygon_position[:,2]) 
    e1 = edge(points,polygon_position[:,2],polygon_position[:,0])
    e2 = edge(points,polygon_position[:,0],polygon_position[:,1])
    w0 = jnp.where(area>0,e0/area,0.) #check if clockwise
    w1 = jnp.where(area>0,e1/area,0.)
    w2 = jnp.where(area>0,e2/area,0.)
    we0 = jnp.where((w0<0.) | (w1<0.) | (w2<0.),0.,w0)
    we1 = jnp.where((w0<0.) | (w1<0.) | (w2<0.),0.,w1)
    we2 = jnp.where((w0<0.) | (w1<0.) | (w2<0.),0.,w2)
    return jnp.stack([we0,we1,we2],axis=1) #[n,3,h,w]
    # w0 = edge(points,polygon_position[:,1],polygon_position[:,2])/area[:,None,None]
    # w1 = edge(points,polygon_position[:,2],polygon_position[:,0])/area[:,None,None]
    # w2 = edge(points,polygon_position[:,0],polygon_position[:,1])/area[:,None,None]
    # e0 = w0.at[(w0<0.) | (w1<0.) | (w2<0.)].set(0.)
    # e1 = w1.at[(w0<0.) | (w1<0.) | (w2<0.)].set(0.)
    # e2 = w2.at[(w0<0.) | (w1<0.) | (w2<0.)].set(0.)
    # e0 = jnp.where((w0<0.) | (w1<0.) | (w2<0.),0.,w0)
    # e1 = jnp.where((w0<0.) | (w1<0.) | (w2<0.),0.,w1)
    # e2 = jnp.where((w0<0.) | (w1<0.) | (w2<0.),0.,w2)
    return jnp.stack([e0,e1,e2],axis=1) #[n,3,h,w]

def getUV_n(points,polygon_position):
    area = getArea_n(polygon_position)[:,None,None]
    e0 = edge(points,polygon_position[:,1],polygon_position[:,2]) 
    e1 = edge(points,polygon_position[:,2],polygon_position[:,3])
    e2 = edge(points,polygon_position[:,3],polygon_position[:,0])
    e3 = edge(points,polygon_position[:,0],polygon_position[:,1])
    w0 = jnp.where(area>0,e0/area,0.) #check if clockwise
    w1 = jnp.where(area>0,e1/area,0.)
    w2 = jnp.where(area>0,e2/area,0.)
    w3 = jnp.where(area>0,e3/area,0.)
    we0 = jnp.where((w0<0.) | (w1<0.) | (w2<0.) | (w3<0.),0.,w0)
    we1 = jnp.where((w0<0.) | (w1<0.) | (w2<0.) | (w3<0.),0.,w1)
    we2 = jnp.where((w0<0.) | (w1<0.) | (w2<0.) | (w3<0.),0.,w2)
    we3 = jnp.where((w0<0.) | (w1<0.) | (w2<0.) | (w3<0.),0.,w3)
    return jnp.stack([we0,we1,we2,we3],axis=1) #[n,3,h,w]


def project(polygon_positions,camera_position,viewport,zNear,zFar):
    target_point = jnp.array([0.,0.,0.])
    view_point = camera_position
    view_up = jnp.array([0.,1.,0.])
    view_plane_normal = target_point-view_point
    # n = -(view_plane_normal)/jnp.linalg.norm(view_plane_normal) # front look at negative z
    # u = (jnp.cross(view_up,n))/jnp.linalg.norm(jnp.cross(view_up,n)) # left hand
    # v = (jnp.cross(n,u))/jnp.linalg.norm(jnp.cross(n,u)) # actual up
    n = -normalize(view_plane_normal)
    u = normalize(jnp.cross(view_up,n))
    v = normalize(jnp.cross(n,u))
    viewMatrix = jnp.array([[u[0],u[1],u[2],-jnp.dot(view_point,u)],
                            [v[0],v[1],v[2],-jnp.dot(view_point,v)],
                            [n[0],n[1],n[2],-jnp.dot(view_point,n)],
                            [0.,0.,0.,1.0]])

    aspect = viewport[0]/viewport[1]
    l = -1.0
    r = 1.0
    b = -1.0
    t = 1.0

    projectMatrix = jnp.array([[2*zNear/(r-l),0.,-(l+r)/(r-l),0.],
                                [0., 2*zNear/(t-b),-(t+b)/(t-b),0.],
                                [0.,0.,(zNear+zFar)/(zNear-zFar),-2*zNear*zFar/(zNear-zFar)],
                                [0.,0.,1.,0.]])

    mvp = (projectMatrix@viewMatrix@polygon_positions.transpose(0,2,1)).transpose(0,2,1)
    mvp = jnp.where(mvp!=0,mvp,1.0)
    projected_polygon_positions = mvp.at[...,0:-1].set(mvp[...,0:-1]/mvp[...,-1][:,:,None])

    return projected_polygon_positions

def get_points_from_angles(elevation, azimuth, distance, degrees=True):
    if isinstance(elevation, float) or isinstance(elevation, int):
        if degrees:
            elevation = jnp.radians(elevation)
            azimuth = jnp.radians(azimuth)
        return jnp.array([
            distance * jnp.cos(elevation) * jnp.cos(azimuth),
            distance * jnp.sin(elevation),
            distance * jnp.cos(elevation) * jnp.sin(azimuth)])
    else:
        if degrees:
            elevation = jnp.pi / 180. * elevation
            azimuth = jnp.pi / 180. * azimuth
    #
        return jnp.stack([
            distance * jnp.cos(elevation) * jnp.cos(azimuth),
            distance * jnp.sin(elevation),
            distance * jnp.cos(elevation) * jnp.sin(azimuth)
            ]).transpose(1, 0)

def get_angles_from_points(point, degrees=True):
    x,y,z = point
    distance = jnp.linalg.norm(point)
    elevation = jnp.arcsin(y/distance)
    azimuth = jnp.arcsin(z/(distance*jnp.cos(elevation)))
    if degrees:
        elevation = jnp.degrees(elevation)
        azimuth = jnp.degrees(azimuth)
    return jnp.array([elevation,azimuth,distance])

def aggregation_alpha(imgs,func):
    if func == 'probabilistic':
        img = imgs[0]
        for img_t in imgs[1:]:
            img = img_t + img - img_t * img
    elif func == 'maximum':
        img = jnp.amax(imgs,axis=0)
    elif func == 'average':
        denominator = (imgs>1e-10).sum(0)
        img = imgs.sum(0) / denominator
    # elif func == 'average':
    #     img = jnp.where(imgs>0,imgs/imgs.sum(0),0)
    elif func == 'summation':
        img = imgs.sum(0)
    return img

def aggregation_rgb(imgs:'[n,h,w,1]',depth:'[n,h,w,1]',func,gamma=1e1)->'[n,h,w,1]':
    if func == 'hard':
        return imgs*(((depth==jnp.max(depth,axis=0)) & (depth != jnp.min(depth,axis=0))))
    elif func == 'softmax':
        depth = (depth-depth.min())/(depth.max()-depth.min()) # normalize
        depth = depth-depth.max() # relative
        return jax.nn.softmax(depth/gamma,axis=0)*imgs
    elif func == 'softras_softmax':
        depth = (depth-depth.min())/(depth.max()-depth.min()) # normalize
        depth = depth-depth.max() # relative
        exp_inv_depth = (jax.lax.stop_gradient(jnp.exp((depth)/gamma))*imgs)
        exp_inv_depth_sum = jax.lax.stop_gradient(jnp.where(exp_inv_depth.sum(0)!=0.,exp_inv_depth.sum(0),1.0))
        return exp_inv_depth / exp_inv_depth_sum
    elif func == 'None':
        return imgs
    else:
        assert False, "only hard or softmax"

batch_aggregation_rgb = vmap(aggregation_rgb,in_axes=(0,0),out_axes=(0))